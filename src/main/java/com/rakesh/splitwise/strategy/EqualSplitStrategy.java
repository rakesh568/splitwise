...
public class EqualSplitStrategy implements SplitStrategy {
    ...
    public void apply(Expense expense, User paidByUser, ExpenseRequest expenseRequest) {
        ...
        int numUsers = expenseRequest.getSplitBreakup().size();
        double equalShare = expense.getAmount() / numUsers;

        // Create DEBIT transactions for each user
        for (ExpenseRequest.Share share : expenseRequest.getSplitBreakup()) {
            ...
            debitTransaction.setAmount(equalShare);
